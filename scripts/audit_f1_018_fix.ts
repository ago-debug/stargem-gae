...
async function run() {
  ...
  const [q1] = await connection.query(`
    ...
    WHERE fiscal_code IS NOT NULL
      AND fiscal_code != ''
      AND (participant_type NOT IN ('DIPENDENTE','INSEGNANTE','PERSONAL_TRAINER') OR participant_type IS NULL)
  ...
  const [q4] = await connection.query(`
    ...
    WHERE m1.fiscal_code IS NOT NULL
      AND m1.fiscal_code != ''
      AND (m1.participant_type NOT IN ('DIPENDENTE','INSEGNANTE','PERSONAL_TRAINER') OR m1.participant_type IS NULL)
      AND (m2.participant_type NOT IN ('DIPENDENTE','INSEGNANTE','PERSONAL_TRAINER') OR m2.participant_type IS NULL)
    LIMIT 5;
  `);
