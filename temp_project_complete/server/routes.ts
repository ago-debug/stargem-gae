  ...
  app.post("/api/workshops", isAuthenticated, checkPermission("/workshops", "write"), async (req, res) => {
    try {
      ...
      if (!force && validatedData.studioId && validatedData.startDate && validatedData.startTime && validatedData.endTime) {
        ...
        if (conflict) {
          return res.status(409).json({
            message: `Conflitto rilevato: lo slot è già occupato da un ${conflict.type === 'course' ? 'corso' : conflict.type === 'booking' ? 'prenotazione' : 'workshop'} (${conflict.name}). Vuoi forzare il salvataggio?`,
            conflict
          });
        }
      }
  ...
  app.post("/api/studio-bookings", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!req.body.force) {
        ...
        if (conflict) {
          return res.status(409).json({
            message: `Conflitto: lo studio è già occupato da "${conflict.name}"`,
