...
export interface IStorage {
  ...
  // Conflict detection
  checkStudioConflict(studioId: number, bookingDate: Date | string, startTime: string, endTime: string, currentBookingId?: number): Promise<{ type: 'booking' | 'course' | 'workshop', name: string } | null>;
...
`);
  ...
  async createStudio(studioData: InsertStudio): Promise<Studio> {
    ...
    const [studio] = await db.select().from(studios).where(eq(studios.id, result.insertId));
    return studio;
  }
  ...
  ): Promise<{ type: 'booking' | 'course' | 'workshop', name: string } | null> {
