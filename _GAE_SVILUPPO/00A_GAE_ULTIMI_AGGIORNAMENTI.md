...
* **[QA VISUALE POST-TEST] Fix Emergenziali per Martedì (AG-043 / AG-045):** A seguito del collaudo visivo e funzionale del layer Calendario/Date, l'elenco di priorità per Martedì è stato codificato e chiuso: 1) Labeling rigoroso della season di default a "25-26"; 2) Auto-switch testuale della "26-27" programmato per i mesi di febbraio; 3) Refactoring UI "OGGI" dinamico (giallo solo sul tile effettivo, e testuale invisibile nelle date fuori-range); 4) Navigazione temporale ibrida (swipe/scroll + active select); 5) Zero-overlap perenne garantito su tutte le attività in griglia; 6) Fedeltà UI 1:1 della Master Table Date Strategiche (assoluto divieto di scomparsa delle righe d'esempio al typing). Tracker allineati ai file personali.
