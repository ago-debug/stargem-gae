...
* **[QA POST-TEST] Fix Emergenziali per Martedì (AG-043):** A seguito del test utente del layer Calendario e Date, lo sprint è stato parzialmente riaperto per accogliere 4 ticket bloccanti e prioritari: 1) Default season *deve* essere costantemente ancorata a "25-26" all'avvio; 2) Switch stagionale a "26-27" da automatizzare al mese di Febbraio di ogni ciclo; 3) Bug visivo (overlap/shinking card) che corrompe la UI per tutte le varabili di attività; 4) Tabella Master Date in cui le righe "template/esempio" collassano e scompaiono scorrettamente al primo inserimento dati dell'utente. Tracker aggiornati.
