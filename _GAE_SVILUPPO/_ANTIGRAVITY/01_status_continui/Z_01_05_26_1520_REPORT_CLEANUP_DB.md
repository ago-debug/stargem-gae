...
## FASE D — Bonifiche Eseguite (Chat_08_Corsi)
Durante la sessione `Chat_08_Corsi` (F1-013-LIGHT) sono state eseguite le seguenti bonifiche:
- **DT → visita_medica:** 2 record storici errati (ID 551, 554) aggiornati ad `activity_type='visita_medica'`.
- **Enrollments:** 1011 record di iscrizione ricollegati ed allineati a `visita_medica`.
- **Prenotazioni fantasma:** Rilevati 2 record anomali con `activity_type='prenotazioni'` (ID 465, 466). La bonifica di questi due record è stata ufficialmente rinviata alla sessione `Chat_22b`.

**Backup Pre-Operazione:** `CHAT08_F1013LIGHT_PRE_UPDATE_DT_20260429.sql` salvato.
