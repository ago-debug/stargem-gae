...
- **Motore AI (Agente Teo)**: Integrato nativamente nel server via *Vercel AI SDK* (`ai@3.4.33` e `@ai-sdk/openai`). Il bridge verso i LLM di OpenAI è protetto a livello server con Sicurezza Hard-RBAC (i tool ereditano dinamicamente il ruolo fisico dell'utente, bloccando alla radice l'accesso ai ruoli client).
- **Osservabilità Enterprise**: Il backend implementa `Sentry` e `PostHog` per product analytics. Integrato log management centralizzato (`winston` in `logs/` con rotazione file). Creato sistema di Disaster Recovery automatico (`scripts/backup-db.sh`).
