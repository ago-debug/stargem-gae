...
> **Ultimo Aggiornamento:** 01 Maggio 2026, 17:25
...
| **Clarissa (CRM & IA)** | Intelligenza Artificiale e Automazioni. | `ai` (Vercel SDK), `server/ai.ts` | 🟡 Fase 2 (Teo) |
...
- **Motore AI (Agente Teo)**: Integrato nativamente nel server via *Vercel AI SDK* (`ai@3.4.33` e `@ai-sdk/openai`). Il bridge verso i LLM di OpenAI è protetto a livello server. Le query dell'AI al database sono incapsulate in `server/ai.ts` tramite funzioni Zod-typed (es. `searchMembers`, `searchCourses`) per garantire la sicurezza del SOT.
- **Osservabilità Enterprise**: Il backend implementa `Sentry` (tracciamento errori globali via `index.ts`) e un wrapper `Redis` fault-tolerant (`server/cache.ts`) con fallback di emergenza nel caso in cui il demone Redis locale crashi.
...
7. Migrazione Dati: Le tabelle strutturali (`custom_lists`) si alterano con *UPDATE in-place*, non con DELETE/INSERT, a causa dei vincoli *ON DELETE CASCADE*.
8. Modello Persone Fluido: Un `member` può avere molteplici cappelli (Studente, Docente, Affittuario). Non frammentare l'anagrafica in più tabelle fisiche per ruolo.

---

## 5. Endpoint API Attivi (Fase di Consolidamento F1)

In seguito alle recenti bonifiche e alla migrazione STI (sessione Chat_08), i principali endpoint attivi e le loro regole sono i seguenti:

- **/api/dashboard/attivita-panoramica**: (Nuovo, F1-019) Cruscotto aggregato per la panoramica attività.
- **/api/activities-summary**: (Modificato, F1-015) Restituisce il riepilogo attività. Applica di default il filtro stagione corrente (`active`), sovrascrivibile con `?seasonId=NN` o `?seasonId=all`.
- **/api/courses?activityType=workshop**: Sostituisce il vecchio endpoint `/api/workshops` (che NON esiste più). Funge da entry point universale filtrato.
- **/api/courses/:id/enrolled-members**: (Preesistente) Pattern flat canonico per recuperare gli iscritti a un corso specifico.
- **/api/chat**: (Nuovo) Endpoint di streaming bidirezionale AI per l'agente copilot "Teo". Elabora i prompt utente e restituisce uno stream di testo e tool calls.
- **/api/ai/generate-promo**: (Nuovo) Endpoint stateless per la generazione strutturata di messaggi promozionali AI-driven (utilizzato dal `MagicPromoButton`).

**Nota Importante (Audit F1-020):** Si documenta ufficialmente che **NON esistono** endpoint singoli di tipo `/api/courses/:id`, `/api/workshops/:id`, `/api/enrollments/:id`. Tutte le interrogazioni devono passare dai listati generali o dai pattern raccordati.
