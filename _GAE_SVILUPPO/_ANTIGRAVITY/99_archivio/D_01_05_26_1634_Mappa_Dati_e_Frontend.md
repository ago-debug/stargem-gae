...
> **Ultimo Aggiornamento:** 01 Maggio 2026, 17:15
...
### 3.3 Gestione Corsi & Iscritti (Aggiornato post F1-017 / Chat_08)
**Conteggi Reali (Stagione 25/26):** Panoramica: 314 corsi totali / 294 attivi. 18 workshop, 13 domeniche, 1 LI con 38 iscritti, 4 campus, 2 allenamenti.

**Mappa Pagine `/attivita`:**
| Pagina / UI | File Source | Endpoint Usato | Stato |
|---|---|---|---|
| Panoramica | `attivita.tsx` | `/api/activities-summary` | ✅ Operativo |
| Wrapper Corsi | `courses.tsx` | `/api/courses?activity_type=corso` | ✅ Operativo |
| Wrapper Workshop | `workshops.tsx` | `/api/courses?activity_type=workshop` | ✅ Operativo |
| Wrapper Domeniche | `sunday-activities.tsx` | `/api/courses?activity_type=domenica_movimento` | ✅ Operativo |
| Wrapper LI | `individual-lessons.tsx` | `/api/courses?activity_type=lezione_individuale` | ✅ Operativo |
| Wrapper Allenamenti | `trainings.tsx` | `/api/courses?activity_type=allenamenti` | ✅ Operativo |
| Wrapper Campus | `campus-activities.tsx` | `/api/courses?activity_type=campus` | ✅ Operativo |

**Pattern dati flat per schede dettaglio:**
- Endpoint `/api/courses/:id/enrolled-members` restituisce flat (NON più nested).
- Tutte le schede canoniche usano destructuring piatto: `data.first_name`, `data.medical_expiry_date`, `data.presenze_count`.
- NO più property innestate tipo `data.member.firstName` o `data.attendances.length`.
