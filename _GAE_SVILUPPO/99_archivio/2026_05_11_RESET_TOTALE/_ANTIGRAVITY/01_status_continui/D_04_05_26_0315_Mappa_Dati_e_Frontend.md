...
> **Ultimo Aggiornamento:** 04 Maggio 2026, 03:15
...
## MAPPATURA PRESENZE E REGISTRO DI CLASSE
1. **Appello Rapido (Bulk Check-in)**:
   - Sviluppata e integrata l'API `POST /api/attendances/bulk` nel `server/routes.ts` gestita tramite `createAttendancesBulk` nel layer storage.
   - Permette il salvataggio massivo delle presenze per tutti gli allievi iscritti al corso in una singola transazione DB.
2. **Visualizzazione a Matrice Pivot**:
   - Sostituita la vecchia lista cronologica con una tabella a doppia entrata nel `CourseUnifiedModal.tsx`.
   - Colonne: le ultime 5 date di lezione reali.
   - Righe: tutti gli allievi attivi. Il backend elabora automaticamente le celle valorizzate coi check-in.
   - Integrazione pulsanti rapidi "Rimuovi" al passaggio del mouse sulle singole presenze.
3. **Trigger di Apertura da Scheda Attività**:
   - Reso operativo il badge del totale presenze ("0 presenze") nella top-bar di `scheda-attivita.tsx`.
   - Il click esegue l'apertura del componente modale `CourseUnifiedModal` impostando la prop `defaultTab` su `"attendances"`, guidando istantaneamente l'utente nella sezione dell'Appello.
