...
## 🔗 File collegati (hub di navigazione)

**Canonici e regole:** [[00_INDEX]] · [[ISTRUZIONI_COWORK_2026_05_05]] · [[ANALISI_MASTER]] · [[00_LEGGIMI]]

**Briefing AG corrente:** [[00_BRIEFING_RIPRESA_2026_05_05]] · [[01_PROMPT_APERTURA_AG]]

**RECAP attivi (in corso / in collaudo):** [[RECAP_01_Quote_e_Promozioni]] · [[RECAP_03_GemTeam]] · [[RECAP_05_GemPass]] · [[RECAP_06_Contabilita]] · [[RECAP_08_Corsi]] · [[RECAP_12B_Gemdario]] · [[RECAP_22_ImportExportBonifica]]

**RECAP completati / stabili:** [[RECAP_02_GemStaff]] · [[RECAP_10_Utenti_GemPortal]] · [[RECAP_24_DBMonitor]]

**RECAP da iniziare:** [[RECAP_04_MedGem]] · [[RECAP_07_Gemory]] · [[RECAP_09_Workshop]] · [[RECAP_11_Campus]] · [[RECAP_13_DomenicheInMovimento]] · [[RECAP_14_BookGem]] · [[RECAP_15_Saggi]] · [[RECAP_16_VacanzeStudio]] · [[RECAP_17_Clarissa]] · [[RECAP_18_GemEvent]] · [[RECAP_19_GemNight]] · [[RECAP_20_MerchSG]] · [[RECAP_23_LogVerifiche]] · [[RECAP_25_KnowledgeBase]]

**Sessioni Analisi recenti:** [[RECAP_00_ChatAnalisi_2026_04_28]] · [[RECAP_00_ChatAnalisi_2026_05_05_BridgeMCP]] · [[RECAP_00_ChatAnalisi_2026_05_05_BrowserMode]] · [[RECAP_00_ChatAnalisi_2026_05_05_ModelliAI]]
...
## DOMANDE APERTE DI GAETANO — RISPOSTE F1 (2026-05-11)

Le 4 domande architetturali di Gaetano (originariamente in `_ANTIGRAVITY/01_status_continui/E_Segnalazioni_DB.md` del 04/05) hanno avuto **risposta ufficiale** nello strategic review del 2026-05-11 (vedi `_ANTIGRAVITY/02_output_protocolli/strategic_review_F1_backend_2026_05_11.md`).

### 1. Tessere in `members` (colonne O-U) — RISPOSTA: debito tecnico, da bonificare
È debito tecnico nato dall'import piatto di GSheets. Un utente ha **più** tessere nel tempo: averle piatte in `members` forza la tabella a tracciare solo "l'ultimo attivo", perdendo lo storico. La Source of Truth deve risiedere **esclusivamente in `memberships`** (3.305 record già pronti).
**Bonifica ad alto rischio se eseguita ORA**: decine di API e UI leggono ancora `members.hasMedicalCertificate` etc. Sequenza obbligata: prima refactor route API per JOIN su `memberships`, poi drop colonne da `members`. Mai nell'altro ordine.

### 2. Certificati medici in `members` (colonne V-W) — RISPOSTA: stesso pattern
Analogo a #1. Source of Truth deve risiedere **esclusivamente in `medical_certificates`** (2.770 record). Stessa sequenza: refactor JOIN prima, drop colonne dopo.

### 3. Colonna A (id legacy) — RISPOSTA: da eliminare appena terminato export Athena
È un ID storico residuo dell'import. Da droppare al completamento del re-import + verifica che nessun export pending lo usi.

### 4. Colonna BA (id vecchio) — RISPOSTA: confermato da droppare
Stesso quesito della 3, AG conferma la proposta di Gaetano: si elimina.

### CONSEGUENZA OPERATIVA
La bonifica della tabella `members` diventa un workstream di 2-3 giorni (post refactor route API). Da pianificare DOPO il re-import dei dati e DOPO il fix UI delle 4 chat dipendenti (vedi Priorità 1b in [[strategic_review_sintesi_2026_05_11]]).

---

## PIANO PRIORITÀ 6-8 SETTIMANE (post-review 2026-05-11)

Sintesi delle due strategic review (F1 backend + F2 frontend). Dettaglio completo in [[strategic_review_sintesi_2026_05_11]].

### Settimane 1-2 (12-25 maggio) — Sblocco utente immediato
1. Investigare bug raggruppamento Planning in 12_Gemdario (Task 3 lista paralleli)
2. Implementare fix → togliere UI FREEZE su Gemdario
3. F1: refactor route API per JOIN su `memberships` e `medical_certificates` (sgancia la lettura tessere/certificati da `members`)
4. F2: esporre i 54 campi Athena nell'anagrafica (PRIORITA 1b — basato sui JOIN nuovi)

### Settimane 3-4 (26 maggio - 8 giugno) — Backend stabilization
5. Smantellamento `routes.ts` modulo per modulo (workstream dedicato F1, no parallelo)
6. In parallelo: estrazione `PaymentModuleConnector` in service backend + `usePricingEngine` frontend

### Settimane 5-6 (9-22 giugno) — Preparazione multi-tenant
...
### Settimane 7-8 (23 giugno - 6 luglio) — Frontend consolidation
11. `ActivityLayout` agnostico per le 5 schede attività
12. Refactor `calendar.tsx` (estrazione hook in `useCalendarGrid`, `usePlanningGrouping`)
13. Bonifica colonne tessere/certificati da `members` (drop dopo i JOIN del passo 3)

### Asincrono (in parallelo, atomici)
- Decisione STRADA A/B per `/importa` (decisione Gaetano)
- Re-import members/memberships/payments (quando Gaetano ha i dati puliti)

### NON FARE in queste 6-8 settimane
- Tab "Incolla Testo" in `/importa` (F2 bocciato — inutile complicazione UX)
- Claude Code Agent Teams in parallelo (F1+F2 bocciato unanime — conflitti git certi)
- Integrazioni AI aggiuntive (F1 raccomanda di consolidare l'esistente prima)
- Refactor automatici massivi (entrambi sconsigliato — solo modulo-per-modulo manuale)
...
Tabelle principali con record reali:
  ...
  team_scheduled_shifts: 17 (turni rimessi manualmente da Gaetano — sezione GemTeam operativa, reimport non più necessario al 2026-05-11)
  team_shift_templates: 1 (stesso: reimport cancellato)
...
Criticità aperte:
  1. enrollmentId = null su ~3.200 pagamenti (collegati a persona ma non al corso specifico)
  2. participation_type non uniforme: 'corso' e 'STANDARD_COURSE' coesistono → uniformare in Chat_08
  3. routes.ts (12k righe) e maschera-input-generale.tsx (4.5k righe) — smantellamento sospeso per sicurezza
  4. Tessere/certificati medici dentro `members` — debito tecnico (vedi sezione "Domande aperte di Gaetano — Risposte F1")
  ~~5. team_scheduled_shifts wipe~~ CHIUSO 2026-05-11 (Gaetano ha rimesso turni manualmente, reimport non necessario)
...
~~PRIORITA 2 — Reimportare turni GemTeam~~ CHIUSO 2026-05-11 (Gaetano ha rimesso turni manualmente, sezione operativa)
