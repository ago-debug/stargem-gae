...
def rewrite_file(filepath, title, role):
    ...
    # We will just write the file completely using a template based on the requirement
    
    # Let's extract the imports from the original
    imports_section = content[:content.find("export default function ")]
    
    template = f"""import {{ useState, useMemo }} from "react";
...
export default function SchedaComponent() {{
...
"""
    ...
    sections = ""
    if role == "domenica":
        sections = domenica_sections
    elif role == "li":
        sections = li_sections
    elif role == "campus":
...
"""
    ...
    full = template + template2
    
    if role == "domenica":
        full = full.replace("SchedaComponent", "SchedaDomenica")
    elif role == "li":
        full = full.replace("SchedaComponent", "SchedaLezioneIndividuale")
    elif role == "campus":
        full = full.replace("SchedaComponent", "SchedaCampus")
        
    with open(filepath, "w") as f:
        f.write(full)

rewrite_file("client/src/pages/scheda-domenica.tsx", "Scheda Domenica in Movimento", "domenica")
rewrite_file("client/src/pages/scheda-lezione-individuale.tsx", "Scheda Lezione Individuale", "li")
rewrite_file("client/src/pages/scheda-campus.tsx", "Scheda Campus", "campus")
