...
async function main() {
  ...
  try {
      ...
      // Helper to process SKU and add enrollment
      ...
      for (const row of data1) {
          ...
          if (!fc && !(name && surname)) continue; // Cannot identify member
          
          let memberId = fc ? memberByFC.get(fc) : null;
          
          // If member doesn't exist but has name+surname, create it to preserve data
          if (!memberId) {
              ...
              const [res] = await connection.execute(
                  `INSERT INTO members (first_name, last_name, fiscal_code, phone, gender, source) VALUES (?, ?, ?, ?, ?, ?)`,
                  [name || '', surname || '', fc, phone, gender, 'BITRIX_AUTO']
              );
      ...
      for (const row of data2) {
          ...
          if (!memberId) {
              ...
              const [res] = await connection.execute(
                  `INSERT INTO members (first_name, last_name, fiscal_code, phone, source) VALUES (?, ?, ?, ?, ?)`,
                  [name || '', surname || '', fc, phone, 'WS_AUTO']
              );
              memberId = res.insertId;
              if (fc) memberByFC.set(fc, memberId);
          }
      ...
      console.log(`Unmatched SKUs skipped: ${skippedSkus.size}`);
