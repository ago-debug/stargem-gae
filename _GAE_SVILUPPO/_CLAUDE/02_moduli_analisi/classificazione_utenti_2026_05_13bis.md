...
aggiornato: 2026-05-13T18:00 ultima\_verifica\_vs\_codice: 2026-05-13 (basato su dialogo Cowork \+ audit AG) validita\_prevista: 2026-06-13 tipo: proposal-modulo priorita: P0-architetturale tags: \[classificazione, utenti, tesseramento, fatturazione, modello-dati, da-validare-team\] fonti:

- "\[\[audit\_F1-004\_flusso\_iscrizioni\_backend\_2026\_05\_12\]\]"  
- "\[\[audit\_F2-003\_flusso\_iscrizioni\_frontend\_2026\_05\_12\]\]"  
- "\[\[proposal\_Quote\_Param\_2026\_05\_12\]\]"  
- "\[\[16\_RECAP\_COMPLETO\_QUOTE\_PARAM\_E\_QUOTE\_CORSI\]\]"  
...
**Documento da rivedere col team.** Prodotto da Cowork dopo dialogo con Gaetano del 13/05/2026. Le sezioni “DA APPROFONDIRE CON TEAM” evidenziano i punti dove serve confronto interno o consulenza fiscale esterna. Collegato a \[\[CHECKLIST\_PROGETTO\]\] Priorità \#6 (SEG-002 rinominazione Anagrafica → Utente) \+ \#3.5 (Quote\_Param, listino parametrico).
...
- SKU “intoccabili” già a sistema: `2526QUOTATESSERA`, **(questi sku sono da considerare SOLO nella visita medica `2526DTYURI`, `2526DTNELLA)`** (vedi \[\[00\_LEGGIMI\]\])
...
**Attività a fatturazione (non mettere fatturazione. In questi casi menzionati facciamo ricevuta istituzionale) ‘ricevuta’ quando tesserato:**
...
⚠️ **DA APPROFONDIRE CON COMMERCIALISTA**: questa regola è fiscalmente corretta? Es. un tesserato che fa una lezione individuale — ricevuta istituzionale o fattura? Risposta di Gaetano “tesserato → ricevuta”, ma vale la pena validare formalmente **(va bene la ricevuta istituzionale per le lezioni individuali ai tesserati).**
...
- Welfare/promo/agevolazioni gestite come **voci di sconto sovrapposte** sulla riga del carrello, NON come livello di listino (cf. \[\[proposal\_Quote\_Param\_2026\_05\_12\]\])
...
Tabella esistente: `company_agreements` (21 record secondo MASTER\_STATUS; 11 secondo RECAP\_01 — ⚠️ valore discordante, da riconciliare).
...
\-- Tabella nuova: external\_payers (paganti esterni non-CRM)

external\_payers (

  id, business\_name, fiscal\_code, vat\_number, address, notes,
...
ALTER TABLE payments ADD COLUMN payer\_id INT;

ALTER TABLE payments ADD COLUMN payer\_type VARCHAR(20); \-- 'member' | 'society' | 'external'

ALTER TABLE payments ADD COLUMN billing\_subject\_id INT;

ALTER TABLE payments ADD COLUMN billing\_subject\_type VARCHAR(20);

ALTER TABLE payments ADD COLUMN document\_type VARCHAR(20); \-- 'ricevuta' | 'fattura' | 'booking\_only'

\-- Modifiche a members (anagrafica)

ALTER TABLE members ADD COLUMN is\_trial BOOLEAN DEFAULT FALSE;

ALTER TABLE members ADD COLUMN is\_commercial\_client BOOLEAN DEFAULT FALSE;

\-- (verificare se esiste già qualcosa di simile prima di aggiungere)

\-- Verificare se necessità di "tessere collettive"

\-- (es. nuova tabella society\_memberships per casi welfare)

### 6.2 Regola tenant\_id

Tutte le nuove tabelle/colonne devono rispettare la **Regola 13** del \[\[00\_LEGGIMI\]\]: `tenant_id` default `'1'`.
...
- **la società non può essere tesserata. La società affitta, riceve fattura, ma non è mai tesserata. Si tesserano solo le persone fisiche.**  
...
Scuola di danza paga per 5 allieve un workshop

-   
- Una scuola di danza acquista 5 workshop destinati a 5 proprie allieve e provvede direttamente al pagamento per loro conto.  
- La fattura viene emessa intestata alla scuola di danza.  
-   
- Successivamente:  
-   
- vengono raccolti i dati anagrafici delle singole ragazze;  
- le allieve vengono inserite su Athena esclusivamente come anagrafiche partecipanti;  
- anche l’anagrafica aziendale della scuola di danza viene registrata su Athena, così da poter associare correttamente la fattura intestata alla scuola.  
-   
-   
- 
...
FITPRIME / WELLHUB  
Attivazione cliente  
L’utente acquista gli ingressi o attiva il proprio abbonamento tramite piattaforma Fitprime / Wellhub.  
Il cliente si presenta presso il centro sportivo con app attiva.  
Accesso e check-in  
All’arrivo il cliente effettua il check-in direttamente tramite app.  
Il personale reception verifica:  
corretto check-in effettuato  
validità dell’accesso  
eventuali limitazioni del piano  
Registrazione ingressi  
Gli ingressi vengono registrati automaticamente tramite la piattaforma.  
È necessario controllare periodicamente la corretta contabilizzazione degli accessi.  
Fatturazione  
La fatturazione viene effettuata periodicamente sulla base degli ingressi registrati e approvati sulla piattaforma.  
Per Fitprime la fatturazione viene effettuata trimestralmente.  
Note operative  
Prima dell’accesso alle attività, l’utente deve:  
effettuare il tesseramento GEOS  
consegnare un certificato medico sportivo valido  
In assenza di tesseramento o certificato medico in corso di validità, non sarà possibile accedere alle attività del centro sportivo.  
Il personale reception deve verificare:  
presenza del tesseramento attivo  
validità del certificato medico  
corretta registrazione nel gestionale interno  
Eventuali servizi extra non inclusi nel piano vengono pagati direttamente dal cliente.

PELLEGRINI / WAI  
Attivazione cliente  
L’utente richiede il pagamento del corso tramite piattaforma welfare Pellegrini / WAI.  
Viene generato un voucher comprensivo del valore del corso e delle eventuali commissioni previste dalla piattaforma.  
Gestione amministrativa  
Il voucher viene verificato dalla segreteria/amministrazione.  
Successivamente viene emessa fattura intestata all’azienda indicata dalla piattaforma welfare.  
Registrazione cliente  
Il cliente viene registrato nel gestionale interno secondo la procedura standard del centro sportivo.  
Note operative  
Prima dell’accesso alle attività, l’utente deve:  
effettuare il tesseramento GEOS  
consegnare un certificato medico sportivo valido  
...
validità del certificato medico  
corretta registrazione nel gestionale interno  
Eventuali servizi extra non inclusi nel voucher vengono pagati direttamente dal cliente.

**Q6. Azienda paga corso di dipendente tesserato — documento fiscale?**

- Congregazione di suore (o altro ente) paga un corso per due ragazze  
-   
- Una congregazione di suore, oppure qualsiasi altro ente o soggetto terzo, paga e omaggia due corsi destinati a due ragazze.  
-   
- In questo caso:  
-   
- le due ragazze vengono regolarmente tesserate;  
- vengono inserite in anagrafica come partecipanti ai corsi;  
- la ricevuta istituzionale viene intestata direttamente alle ragazze, in quanto beneficiarie del corso;  
- come previsto per tutti i corsi, viene richiesto il certificato medico.  
- 

**Q7. Il pagante deve essere sempre un soggetto registrato nel gestionale?**

Se un individuo paga per un’attività istituzionale, non è necessario che sia presente nel gestionale, poiché la ricevuta deve essere intestata al beneficiario dell’attività.

Se invece il pagamento riguarda un’attività commerciale, il soggetto che paga deve essere presente nel gestionale, perché la fattura deve essere intestata correttamente a lui.

**Q8. L’intestatario fiscale coincide sempre col pagante?**

No.

Nelle attività istituzionali, l’intestatario della ricevuta è il beneficiario dell’attività, anche se il pagamento viene effettuato da un’altra persona.

Nelle attività commerciali, invece, l’intestatario fiscale della fattura deve coincidere con il soggetto che sostiene il pagamento.
...
2. **Documento fiscale split** (azienda paga corso di dipendente tesserato): è fattura+ricevuta o solo ricevuta dipendente? È il commercialista la fonte di verità : **gia menzionato nei punti 6 e 7\.**  
3. **Merchandising con ricevuta istituzionale**: **sempre fattura anche se è un tesserato**  
4. **Lezioni individuali**: nella lista “tesserato” sono “ricevuta”; nella lista “non-tesserato” sono “fattura”. Confermare che è davvero questa la regola operativa (potrebbe esserci complicanza fiscale) **e' corretto come hai scritto**  
...
6. **Tessere multiple in famiglia**: scontistica fratelli/sorelle già in essere? Come va modellata? (Gaetano: pattern “doppio tesseramento famiglia” non scelto, ma esiste sconto familiare?) **sconto che si applica è quello di 2 o 3 o più corsi, quindi ognuno sarà tesserato.**  
...
4. **Numero record `company_agreements`** reale vs MASTER\_STATUS (21) vs RECAP\_01 (11) — quale è corretto  
...
6. **UI di gestione listino**: esiste già una pagina amministrativa per gestire price\_rules / quote / promo? Dove?  
...
3. Cowork lancia **Subagent Ricerca** per verificare nel codice attuale (\[\[audit\_F1-004\_flusso\_iscrizioni\_backend\_2026\_05\_12\]\]) i punti “DA VERIFICARE” della sezione 9  
...
*12 . PAGAMENTI MULTIPLI*

*12.1 una persona acquista per più persone esempio la mamma compra per due figlie e per se stessa. Dovremo fare una sola ricevuta a chi effettua il pagamento, bisogna fare che nel data base ci sia il collegamento ai nominativi che rientrano in quel pagamento e per quali attività.*  
*La persona che paga dovrà compilare la domanda di tesseramento per tutti. Sarebbe utile che fosse un’unica domanda di tesseramento con la possibilità di inserire tutti i nominativi dei figli.*  
*La stessa procedura dovrà avvenire quando il genitore acquista online per un figlio, corso, campus, workshop …. la ricevuta va fatta al genitore.*

*La stessa procedura dovrà avvenire quando il genitore acquista con bonifico per uno o più figli, corso, campus, workshop …. la ricevuta va fatta al genitore.*  
*Importante, il collegamento nel data base a tutti è fondamentale. Nei corsi dovrà comparire il nome di chi partecipa e il pagamento anche se la ricevuta sarà stata emessa solo al genitore che ha pagato. Nell’anagrafica dovrà esserci che la tessera è stata pagata o rinnovata, quindi la validità sarà confermata anche se il pagamento è stato effettuato dal genitore.*

*13\. ACQUISTO GIFT* 

***La Gift Card può essere acquistata sia online sia in sede e il pagamento può avvenire con qualsiasi metodo. La ricevuta deve essere sempre emessa al momento del pagamento a nome di chi acquista la Gift Card: se l’acquirente è tesserato si emette ricevuta istituzionale, mentre se non è tesserato si emette ricevuta generica da consegnare successivamente all’amministrazione per l’emissione della fattura, come avviene per l’abbigliamento.***

***La Gift Card copre esclusivamente la quota corso; la quota tessera deve sempre rimanere separata, viene pagata al momento della formalizzazione dell’iscrizione ed è registrata insieme alla compilazione della domanda di tesseramento.***

***Il file Gift Card deve essere compilato correttamente in tutte le sue parti: dalla colonna A alla colonna M la compilazione è obbligatoria al momento dell’acquisto, mentre dalla colonna N alla colonna S va completata nelle fasi successive di gestione e utilizzo della Gift Card. Tutte le colonne devono essere sempre aggiornate e non lasciate vuote. Devono inoltre essere presenti e correttamente registrati in anagrafica Athena sia i dati di chi acquista la Gift Card sia quelli di chi la utilizza.***

***Quando la Gift Card viene utilizzata, viene considerata come metodo di pagamento; se non copre l’intera quota corso, la parte restante viene registrata come saldo. Elisa registrerà come entrata soltanto la quota saldo.***

***Esempio: quota corso €395 e Gift Card €200. La ricevuta dovrà riportare quota tessera €25, quota acconto tramite Gift Card €200 e quota saldo €195, per un totale ricevuta di €220. Alla ricevuta devono sempre essere allegati la Gift Card e il post-it descrittivo.***

***14 FOGLIO PER LA DETRAZIONE FISCALE***

***nel gestionale nella sezione anagrafica è stato già menzionato come documento da generare e consegnare a chi lo richiede.***  
***Questo è un appunto di promemoria per ricordare che all’interno dei documenti di una persona dovrà esserci il modulo per la detrazione fiscale richiesto dai genitori per le spese fatte per i figli.***