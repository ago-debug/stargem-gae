...
async function run() {
  ...
  console.log("--- BACKUP INIZIALE ---");
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS courses_backup_op1235_${ts} AS SELECT * FROM courses`));
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS enrollments_backup_op1235_${ts} AS SELECT * FROM enrollments`));
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS memberships_backup_op1235_${ts} AS SELECT * FROM memberships`));
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS medical_certs_backup_op1235_${ts} AS SELECT * FROM medical_certificates`));
    ...
    await db.transaction(async (tx) => {
      ...
      for (const row of members1) {
        ...
        await tx.execute(sql`
          INSERT INTO memberships (member_id, membership_number, season_id, status, membership_type, issue_date, notes, data_quality_flag)
          VALUES (${row.member_id}, ${mNum}, 1, 'active', 'ENDAS', ${row.enrollment_date}, 'importato da bonifica storico Chat22b', 'da_verificare');
        `);
        insertedOp1++;
      }
      console.log(`OP1 (Tessere): Inserite ${insertedOp1} righe (saltati gli orfani senza CF).`);
