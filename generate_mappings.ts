...
const generateMappings = () => {
        ...
        if (item && typeof item === 'object') {
            try {
                ...
                if (cols && Object.keys(cols).length > 0) {
                    tables.push(key);
                    colsByTable[key] = Object.keys(cols);
                }
            } catch(e) {}
        }
    }
    ...
    tables.sort().forEach(t => {
        ...
        if (!label) {
            // format camelCase to readable: teamWorkSchedules -> Team Work Schedules
    ...
    // Dictionary for common columns
    const commonCols: Record<string, {label: string, loc: string}> = {
    ...
    const existingColumnMappings: Record<string, Record<string, { label: string, location: string }>> = {
      "members": {
    ...
    console.log("Mappings generated successfully to ./client/src/config/db-monitor-mapping.ts");
};
