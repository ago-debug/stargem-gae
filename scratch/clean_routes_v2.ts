...
// Collect statements to remove first
const stmtsToRemove: any[] = [];

// Iterate only top-level statements in the source file
...
for (const stmt of stmts) {
  if (stmt.getKind() === SyntaxKind.ExpressionStatement) {
    ...
    const callExpr = exprStmt.getExpressionIfKind(SyntaxKind.CallExpression);
