...
export default function SchedaWorkshop() {
    ...
    const workshopIdRaw = searchParams.get("workshopId");
    ...
    const { data: attendances, isLoading: attendancesLoading } = useQuery<Attendance[]>({ queryKey: ["/api/workshop-attendances"] });

    const { sortConfig, handleSort, sortItems, isSortedColumn } = useSortableTable<any>("lastName");
    if (workshopsLoading || membersLoading || enrollmentsLoading || paymentsLoading || attendancesLoading) {
        return (
        ...
        return (
            <div className="p-6 md:p-8 mx-auto">
                <div className="bg-background p-6 rounded-xl border shadow-sm">
                    <h1 className="text-2xl font-bold text-foreground">Scheda Workshop</h1>
                    <p className="text-muted-foreground mt-2">Parametro <code>workshopId</code> mancante o non valido nell’URL.</p>
                    <div className="mt-4">
    ...
    if (!workshop) {
        return (
            <div className="p-6 md:p-8 mx-auto">
                <div className="bg-background p-6 rounded-xl border shadow-sm">
                    <h1 className="text-2xl font-bold text-foreground">Scheda Workshop</h1>
                    <p className="text-muted-foreground mt-2">Workshop non trovato per <code>workshopId={String(workshopId)}</code>.</p>
                    <div className="mt-4">
                        <Button variant="outline" onClick={() => setLocation("/iscritti_per_attivita")}>
                            Torna a Iscritti per Attività
                        </Button>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-3">
                                    Scheda Workshop {workshop ? `- ${workshop.name}` : ''}
                                </h1>
                    ...
                    {workshop && (
                        <div className="flex flex-wrap gap-3 pt-2">
                            <Badge variant="outline" className="bg-muted border-border text-muted-foreground font-medium px-3 py-1 flex items-center gap-1.5">
                                ...
                                SKU: {workshop.sku || 'N/A'}
                            </Badge>
                            {workshop.dayOfWeek && workshop.startTime && (
                                <Badge variant="outline" className="bg-muted border-border text-muted-foreground font-medium px-3 py-1 flex items-center gap-1.5">
                                    <Clock className="w-3.5 h-3.5" />
                                    {workshop.dayOfWeek} {workshop.startTime} - {workshop.endTime}
                                </Badge>
                            )}
                                ...
                                <TableRow>
                                    <TableCell colSpan={8} className="text-center py-12 text-muted-foreground">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p>Nessun iscritto trovato per questo workshop.</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
