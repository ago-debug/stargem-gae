...
export default function SchedaCorso() {
  ...
  const enrolledMembersData = (enrolledMembersRaw || []).map((data: any) => {
    ...
    const paymentStatusBadge = hasPaidPayments ? (
      ...
      <Badge
        variant="destructive"
        className="bg-red-500/10 text-red-700 hover:bg-red-500/20 shadow-none border-0"
      >
        In Sospeso
      </Badge>
    ) : (
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden">
      ...
      <div className="shrink-0 flex items-center justify-between gap-4 flex-wrap pb-2 pt-2 px-4 border-b border-black/5">
        <div className="flex flex-col gap-3 w-full">
          ...
          {course && (
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              <Badge
                variant="outline"
                className="bg-gold/10 border-gold/30 text-gold-foreground font-medium px-3 py-1 flex items-center gap-1.5"
              >
                  ...
                  sortedEnrolledMembersData.map((data: any) => {
                    ...
                    if (membership_expiry_date) {
                      ...
                      if (isExpired) {
                        cardExpiryText = (
                          <Badge
                            variant="destructive"
                            className="bg-red-500/10 text-red-700 hover:bg-red-500/20 shadow-none border-0 gap-1"
                          >
                            <XCircle className="w-3.5 h-3.5" /> Scaduta (
                            {isValidCardDate
                        ...
                        cardExpiryText = (
                          <Badge className="bg-green-500/10 text-green-700 hover:bg-green-500/20 shadow-none border-0 gap-1">
                            <CheckCircle2 className="w-3.5 h-3.5" /> Valida (
                            {isValidCardDate
                              ? expiryDate.toLocaleDateString("it-IT")
                              : ""}
                            )
                    ...
                    if (medical_expiry_date) {
                      ...
                      if (medical_status === "valid") {
                        certExpiryText = (
                          <Badge className="bg-green-500/10 text-green-700 hover:bg-green-500/20 shadow-none border-0 gap-1">
                            <CheckCircle2 className="w-3.5 h-3.5" /> Valido (
                            {formattedDate})
                          </Badge>
                        );
                      } else if (
                        ...
                        certExpiryText = (
                          <Badge className="bg-yellow-500/10 text-yellow-700 hover:bg-yellow-500/20 shadow-none border-0 gap-1">
                            <AlertTriangle className="w-3.5 h-3.5" /> In
                        ...
                        certExpiryText = (
                          <Badge
                            ...
                            className="bg-red-500/10 text-red-700 hover:bg-red-500/20 shadow-none border-0 gap-1"
                          >
                            <XCircle className="w-3.5 h-3.5" /> Scaduto (
                            {formattedDate})
                          </Badge>
                        );
                      }
