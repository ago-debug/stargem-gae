...
import type { SingleLesson, Member, Enrollment, Payment, Attendance } from "@shared/schema";
import { buildEnrolledMembersData } from "@/lib/enrollments";

export default function SchedaLezioneSingola() {
    ...
    const { data: items, isLoading: itemsLoading } = useQuery<SingleLesson[]>({ queryKey: ["/api/single-lessons"] });
    const { data: members, isLoading: membersLoading } = useQuery<{ members: Member[] }>({ queryKey: ["/api/members"] });
    const { data: enrollments, isLoading: enrollmentsLoading } = useQuery<Enrollment[]>({ queryKey: ["/api/enrollments?type=lezione-singola"] });
        ...
        return (
            <div className="p-6 md:p-8 max-w-7xl mx-auto">
                <div className="bg-white p-6 rounded-xl border shadow-sm">
                    <h1 className="text-2xl font-bold text-slate-800">Scheda Lezione Singola</h1>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight text-slate-800 flex items-center gap-3">
                                    Scheda Lezione Singola {item ? `- ${item.name}` : ''}
                                </h1>
