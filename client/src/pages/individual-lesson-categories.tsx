...
import type { IndividualLessonCategory, InsertIndividualLessonCategory } from "@shared/schema";

export default function IndividualLessonCategories() {
  const { toast } = useToast();
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<IndividualLessonCategory | null>(null);

  const { data: categories, isLoading } = useQuery<IndividualLessonCategory[]>({
    queryKey: ["/api/individual-lesson-categories"],
  });

  const createMutation = useMutation({
    mutationFn: async (data: InsertIndividualLessonCategory) => {
      await apiRequest("POST", "/api/individual-lesson-categories", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/individual-lesson-categories"] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: InsertIndividualLessonCategory }) => {
      await apiRequest("PATCH", `/api/individual-lesson-categories/${id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/individual-lesson-categories"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      await apiRequest("DELETE", `/api/individual-lesson-categories/${id}`, undefined);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/individual-lesson-categories"] });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: InsertIndividualLessonCategory = {
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-foreground" data-testid="text-page-title">Categorie Lezioni Individuali</h1>
            <p className="text-muted-foreground text-sm">Organizza le lezioni individuali per categorie e sottocategorie</p>
          </div>
        </div>
        <Button 
          onClick={() => {
          ...
          data-testid="button-add-individual-lesson-category"
        >
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-medium">Struttura Categorie Lezioni Individuali</h2>
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                  ...
                  <div className="flex items-center justify-between p-3 rounded-md hover-elevate active-elevate-2 border border-border">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                      <Button
                        ...
                        data-testid={`button-edit-individual-lesson-category-${category.id}`}
                      >
                      ...
                      <Button
                        ...
                        data-testid={`button-delete-individual-lesson-category-${category.id}`}
                      >
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                            <Button
                              ...
                              data-testid={`button-edit-individual-lesson-category-${child.id}`}
                            >
                              <Edit className="w-4 h-4" />
                            </Button>
                            <Button
                              ...
                              data-testid={`button-delete-individual-lesson-category-${child.id}`}
                            >
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                data-testid="input-individual-lesson-category-name"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={3}
                data-testid="input-individual-lesson-category-description"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select name="parentId" defaultValue={editingCategory?.parentId?.toString() || "none"}>
                <SelectTrigger data-testid="select-individual-lesson-category-parent">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                data-testid="input-individual-lesson-category-color"
              />
            </div>
            ...
            <DialogFooter>
              <Button
                ...
                data-testid="button-cancel-individual-lesson-category"
              >
                Annulla
              </Button>
              <Button 
                ...
                data-testid="button-submit-individual-lesson-category"
              >
