...
type MemberFormData = Omit<Partial<InsertMember>, 'dateOfBirth' | 'cardIssueDate' | 'cardExpiryDate' | 'entityCardIssueDate' | 'entityCardExpiryDate' | 'medicalCertificateExpiry'> & {
  dateOfBirth?: string;
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        cardIssueDate: formatDateForInput(member.cardIssueDate),
        cardExpiryDate: formatDateForInput(member.cardExpiryDate),
        notes: member.notes || "",
        categoryId: member.categoryId || undefined,
        hasMedicalCertificate: member.hasMedicalCertificate || false,
        medicalCertificateExpiry: formatDateForInput(member.medicalCertificateExpiry),
            ...
            <div className="flex items-center gap-2">
              {!isNewMember && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setLocation(`/scheda-contabile?memberId=${memberId}`)}
                  className="text-orange-700 border-orange-200 hover:bg-orange-50 mr-2"
                  title="Vai alla Scheda Contabile"
                >
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  {memberId && (
                    <Button
                      size="sm"
                      onClick={() => setLocation(`/pagamenti?memberId=${memberId}`)}
                    >
                      <Plus className="w-4 h-4 mr-2" />
                      Nuovo Pagamento
                    </Button>
