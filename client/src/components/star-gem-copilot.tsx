...
export function StarGemCopilot({ onSelect, className = "" }: StarGemCopilotProps) {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52 border-amber-200 dark:border-amber-900/50/50 bg-background/95 backdrop-blur-sm">
                <div className="px-2 py-1.5 text-xs font-semibold text-amber-600 border-b border-amber-100 dark:border-amber-900/50 flex items-center gap-2">
                    <Sparkles className="w-3 h-3" />
                    StarGem CoPilot
                </div>
                <DropdownMenuItem onClick={() => handleSelect("ChatGPT")} className="gap-2 cursor-pointer focus:bg-emerald-50 dark:bg-emerald-950/20 focus:text-emerald-700">
                    <MessageSquare className="w-4 h-4 text-emerald-500" />
                    <span>ChatGPT Assistant</span>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleSelect("Gemini")} className="gap-2 cursor-pointer focus:bg-blue-50 dark:bg-blue-950/20 focus:text-blue-700">
